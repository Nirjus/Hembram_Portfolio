...
const Input = ({required=false, placeholder, state, setState, label, labelSupportedText}: Props) => {
   
  return (
    <div className=" space-y-3 md:mb-7 mb-6">
    <label htmlFor={label} className=" font-mono pl-3 md:text-lg  ">
      • <span className=' capitalize'>{label.split(/(?=[A-Z])/).join(' ')}</span>{required && <span className=" text-red-500">*</span>} {labelSupportedText &&
